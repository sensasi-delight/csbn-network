...
function createOrgs() {
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/courier/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP"
  ./organizations/ccp-generate.sh $CH1
}
...
function networkDown() {
  ...
  if [ "$MODE" == "demolition" ]; then
    ...
    for ORG in ${ORGS[@]}
    do
      docker run --rm -v $(pwd):/data busybox sh -c "cd /data && rm -rf organizations/fabric-ca/${ORG}/msp organizations/fabric-ca/${ORG}/tls-cert.pem organizations/fabric-ca/${ORG}/ca-cert.pem organizations/fabric-ca/${ORG}/IssuerPublicKey organizations/fabric-ca/${ORG}/IssuerRevocationPublicKey organizations/fabric-ca/${ORG}/fabric-ca-server.db"
    done
...
COMPOSE_FILE_BASE=docker/docker-compose-csbn.yaml
# docker-compose.yaml file if you are using couchdb
# certificate authorities compose file
# COMPOSE_FILE_CA=docker/docker-compose-ca.yaml
COMPOSE_FILE_CA=docker/docker-compose-ca-csbn.yaml
...
while [[ $# -ge 1 ]] ; do
  ...
  -cai )
    ...
    ;;
  -ch1 )
    COMPOSE_FILE_BASE=docker/docker-compose-csbn-ch1.yaml
    COMPOSE_FILE_CA=docker/docker-compose-ca-csbn-ch1.yaml
...
elif [ "$MODE" == "demolition" ]; then
  warnln "Demolish network, all stored data will deleted"
